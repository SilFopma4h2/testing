...
def generate_receipt_code():
    """Generate a unique receipt code for conservation fees."""
    import random
    import string
    # 10-character alphanumeric, uppercase
...
def generate_reference_code(payment_type, project_code=None):
    """Generate a structured reference code for payments and donations
    
    Args:
        payment_type: Type of payment ('DON' for donation, 'PAC' for package/conservation fee)
        project_code: Optional project code (e.g., 'MG' for Mangroves)
    
    Returns:
        A formatted reference code like 'DON-MG-2025-1234'
    """
    import datetime
    import random
    ...
    if payment_type.upper() == 'DON':
        prefix = 'DON'
    elif payment_type.upper() == 'PAC':
        prefix = 'PAC'
    else:
        prefix = 'PMT'  # Generic payment
    
    # Format with project code if provided
    if project_code:
        return f"{prefix}-{project_code.upper()}-{year}-{sequence}"
    else:
        return f"{prefix}-{year}-{sequence:06d}"  # 6 digits with leading zeros
...
def donation_form():
    try:
        ...
        # Generate structured reference code
        project = data.get('projectSelection', 'general')
        project_code = None
        if project == 'coral-restoration':
            project_code = 'CR'
        elif project == 'iweni-lmma':
            project_code = 'IL'
        elif project == 'eco-tourism':
            project_code = 'ET'
        elif project == 'marine-awareness':
            project_code = 'MA'
        elif project == 'mangrove-planting':
            project_code = 'MG'
            
        reference_code = generate_reference_code('DON', project_code)
        ...
        donation = Donation(
            ...
            newsletter=data.get('newsletter', False),
            user_id=session.get('user_id'),  # Link to logged-in user if available
            transaction_id=reference_code  # Use the structured reference code
        )
        ...
        payment = Payment(
            ...
            gateway_reference=reference_code  # Store structured reference in payment as well
        )
        ...
        <li>Reference Number: <strong>{reference_code}</strong></li>
        ...
        <p>Please keep your reference number ({reference_code}) for future inquiries.</p>
        <p>Best regards,<br>Hope Foundation Team</p>
        """
        ...
        return jsonify({
            'success': True, 
            'message': f'Thank you! Please send ${amount} worth of {data["paymentMethod"].title()} to complete your donation. Your reference number is {reference_code}. Check your email for wallet address details.',
            'amount': amount,
...
def create_conservation_fee():
    ...
    try:
        ...
        # Generate structured reference code for conservation fee (PAC)
        reference_code = generate_reference_code('PAC')
        ...
        payment = Payment(
            ...
            gateway_reference=reference_code
        )
        db.session.add(payment)
        db.session.flush()
        ...
        fee = ConservationFee(
            ...
            amount=amount,
            payment_method=payment_method,
            transaction_id=reference_code,  # Use structured reference as transaction_id
        ...
        <ul>
          ...
          <li><strong>Reference Number:</strong> <strong>{reference_code}</strong></li>
          <li><strong>Receipt Code:</strong> {fee.receipt_code}</li>
        ...
        <a href="{request.host_url.rstrip('/')}/api/conservation/verify/{fee.receipt_code}">{request.host_url.rstrip('/')}/api/conservation/verify/{fee.receipt_code}</a></p>
        ...
        return jsonify({
            'success': True,
            'message': f'Conservation fee recorded. Your reference number is {reference_code}. Complete crypto transfer using the provided wallet address.',
            'referenceCode': reference_code,
            'receiptCode': fee.receipt_code,
